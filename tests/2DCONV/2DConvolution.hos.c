...
void convolution2D_cpu(int ni, int nj, const double *A, double *B)
{
    // 卷积核系数，与核函数中一致
...
int check_convolution2D(int ni, int nj, double *B_gold, double *B)
{
    int errNum = 0;
...
// 传入的参数根据实际算子需要的参数来调整
void save_data(int ni,int nj,uint64_t *before_hot_data, uint64_t *after_hot_data, uint64_t timeDsp,uint64_t  timeCpu,int clusterId, char *devProgram, int nthreads,char *kernel)
{
    FILE *file = fopen("tests/2DCONV/2DConvolution_events.txt", "a");
    if (file == NULL)
    {
    ...
    fprintf(file, "%d,%d,%s,%s,%d,Difference,", clusterId, ni*nj, devProgram,kernel, nthreads);
...
int main(int argc, char **argv)
{
    int retc;
    int clusterId = 1;
    int ni = 256;      // 行数，默认4096
    int nj = 256;      // 列数，默认4096
    char *devProgram = "operators/2DCONV/2DConvolution.dev.dat";
    int nthreads = 1;
    char *kernel = "convolution2D_kernel";
    char info[256];
    ...
    size_t bufSize = (size_t)ni * nj * sizeof(double);
    double *A = (double *)hthread_malloc(clusterId, bufSize, HT_MEM_RO); M_checkMalloc(A);
    double *B = (double *)hthread_malloc(clusterId, bufSize, HT_MEM_RW); M_checkMalloc(B);
    double *B_gold = (double *)malloc(bufSize); M_checkMalloc(B_gold);
    ...
    {
        // 保存数据
        save_data(ni ,nj,before_hot_data, after_hot_data, timeDev, timeGold, clusterId, devProgram, nthreads,kernel);
        printf("WallTime convolution2D_kernel (DSP/CPU): %fs / %fs\n", timeDev / 1e6, timeGold / 1e6);
        return 0;
    }
