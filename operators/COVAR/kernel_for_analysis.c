...
__global__ void covar_kernel1(int m, int n, float *mean, float *data)
{
    // 获取当前线程的 ID 和线程组大小
...
__global__ void covar_kernel2(int m, int n, float *mean, float *data)
{
    // 获取当前线程的 ID 和线程组大小
    int thread_id = get_thread_id();   // 线程 ID
    int group_size = get_group_size(); // 线程总数
...
__global__ void covar_kernel3(int m, int n, float *symmat, float *data)
{
    // 获取线程ID和线程总数
