 ...
 ************************************/
void jacobi1D_cpu1(int n, double *A, double *B)
{
    for (int i = 1; i < n - 1; i++) {
...
void jacobi1D_cpu2(int n, double *A, double *B)
{
    for (int j = 1; j < n - 1; j++) {
        A[j] = B[j];
    }
...
int check_result(int n, double *A_host, double *A_dev, double *B_host, double *B_dev)
{
    int errNum = 0;
    for (int i = 0; i < n; i++) {
        if (percentDiff(A_host[i], A_dev[i]) > PERCENT_DIFF_ERROR_THRESHOLD) {
            if (errNum < 10) {
                fprintf(stderr, "A diff @[%d] : H=%.6f  D=%.6f\n",
                        i, A_host[i], A_dev[i]);
            }
            errNum++;
        }
        if (percentDiff(B_host[i], B_dev[i]) > PERCENT_DIFF_ERROR_THRESHOLD) {
            if (errNum < 10) {
                fprintf(stderr, "B diff @[%d] : H=%.6f  D=%.6f\n",
                        i, B_host[i], B_dev[i]);
            }
            errNum++;
        }
...
{
    FILE *fp = fopen("tests/JACOBI1D/jacobi1D_events.txt", "a");
...
int main(int argc, char **argv)
{
    ...
    int    tsteps      = 5;  // TSTEPS
    int    nthreads    = 1;
    char  *devProgram  = "operators/JACOBI1D/jacobi1D.dev.dat";
    char  *kernel1     = "jacobi1D_kernel1";
    char  *kernel2     = "jacobi1D_kernel2";
    ...
    /* device */
    double  *A_d  = (double *)hthread_malloc(clusterId, sizeVec, HT_MEM_RW);
    ...
    /* host */
    double *A_h = (double *)malloc(sizeVec);
    ...
    for (int t = 0; t < tsteps; t++) {
        st = getCurrentTimeMicros();
        jacobi1D_cpu1(n, A_h, B_h);
        ed = getCurrentTimeMicros(); tCpu1 += ed - st;

        st = getCurrentTimeMicros();
        jacobi1D_cpu2(n, A_h, B_h);
    ...
    if (err != 0) {
        fprintf(stderr, "JACOBI1D test FAILED!\n");
    } else {
        save_data("JACOBI1D", n, before1, after1, tDsp1, tCpu1,
                  clusterId, devProgram, nthreads, kernel1);
        save_data("JACOBI1D", n, before2, after2, tDsp2, tCpu2,
                  clusterId, devProgram, nthreads, kernel2);
        printf("WallTime JACOBI1D_kernel1 (DSP/CPU): %fs / %fs\n",
               tDsp1/1e6, tCpu1/1e6);
        printf("WallTime JACOBI1D_kernel2 (DSP/CPU): %fs / %fs\n",
               tDsp2/1e6, tCpu2/1e6);
    }
