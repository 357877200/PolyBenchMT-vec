 ...
 ************************************/
void lu_cpu1(int n, int k, double *A)
{
    for (int j = k + 1; j < n; j++) {
...
void lu_cpu2(int n, int k, double *A)
{
    for (int i = k + 1; i < n; i++) {
        for (int j = k + 1; j < n; j++) {
...
{
    FILE *fp = fopen("tests/LU/lu_events.txt", "a");
...
int main(int argc, char **argv)
{
    ...
    int    n           = 1024; // N
    int    nthreads    = 1;
    char  *devProgram  = "operators/LU/lu.dev.dat";
    char  *kernel1     = "lu_kernel1";
    char  *kernel2     = "lu_kernel2";
    ...
    if (!A_d || !before1 || !after1 || !before2 || !after2) {
    ...
    uint64_t args2[5];
    args2[0] = (uint64_t)n;
    args2[1] = 0; // k, to be set in loop
    args2[2] = (uint64_t)A_d;
    ...
    if (err != 0) {
        fprintf(stderr, "LU test FAILED!\n");
    } else {
        save_data("LU", n, before1, after1, tDsp1, tCpu1,
                  clusterId, devProgram, nthreads, kernel1);
        save_data("LU", n, before2, after2, tDsp2, tCpu2,
                  clusterId, devProgram, nthreads, kernel2);
        printf("WallTime LU_kernel1 (DSP/CPU): %fs / %fs\n",
               tDsp1 / 1e6, tCpu1 / 1e6);
        printf("WallTime LU_kernel2 (DSP/CPU): %fs / %fs\n",
               tDsp2 / 1e6, tCpu2 / 1e6);
    }
