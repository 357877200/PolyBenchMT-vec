 ...
 ************************************/
void mvt_cpu1(int n, double *A, double *x1, double *y1)
{
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < n; j++) {
...
void mvt_cpu2(int n, double *A, double *x2, double *y2)
{
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < n; j++) {
 ...
 ************************************/
void init_array(int n, double *A, double *x1, double *x2, double *y1, double *y2)
{
    for (int i = 0; i < n; i++) {
        x1[i] = ((double)i) / n;
        x2[i] = ((double)i + 1) / n;
...
int check_result(int n, double *x1_host, double *x1_dev, double *x2_host, double *x2_dev)
{
    int errNum = 0;
    for (int i = 0; i < n; i++) {
        if (percentDiff(x1_host[i], x1_dev[i]) > PERCENT_DIFF_ERROR_THRESHOLD) {
            if (errNum < 10) {
                fprintf(stderr, "x1 diff @[%d] : H=%.6f  D=%.6f\n",
                        i, x1_host[i], x1_dev[i]);
            }
            errNum++;
        }
        if (percentDiff(x2_host[i], x2_dev[i]) > PERCENT_DIFF_ERROR_THRESHOLD) {
            if (errNum < 10) {
                fprintf(stderr, "x2 diff @[%d] : H=%.6f  D=%.6f\n",
                        i, x2_host[i], x2_dev[i]);
            }
            errNum++;
        }
    }
    if (errNum) {
        double fail_percent = (100.0 * errNum) / (double)(2 * n);
...
{
    FILE *fp = fopen("tests/MVT/mvt_events.txt", "a");
...
int main(int argc, char **argv)
{
    ...
    int    n           = 1000; // N
    int    nthreads    = 1;
    char  *devProgram  = "operators/MVT/mvt.dev.dat";
    char  *kernel1     = "mvt_kernel1";
    char  *kernel2     = "mvt_kernel2";
    ...
    double  *x1_d = (double *)hthread_malloc(clusterId, sizeVec, HT_MEM_RW);
    double  *x2_d = (double *)hthread_malloc(clusterId, sizeVec, HT_MEM_RW);
    double  *y1_d = (double *)hthread_malloc(clusterId, sizeVec, HT_MEM_RW);
    double  *y2_d = (double *)hthread_malloc(clusterId, sizeVec, HT_MEM_RW);
    ...
    if (!A_d || !x1_d || !x2_d || !y1_d || !y2_d || !before1 || !after1 || !before2 || !after2) {
        fprintf(stderr, "device malloc failed\n"); return 1;
    }

    /* host */
    double *A_h  = (double *)malloc(sizeMat);
    double *x1_h = (double *)malloc(sizeVec);
    double *x2_h = (double *)malloc(sizeVec);
    double *y1_h = (double *)malloc(sizeVec);
    double *y2_h = (double *)malloc(sizeVec);

    if (!A_h || !x1_h || !x2_h || !y1_h || !y2_h) {
        fprintf(stderr, "malloc host fail\n"); return 1;
    }

    /* init data */
    init_array(n, A_h, x1_h, x2_h, y1_h, y2_h);
    
    memcpy(A_d, A_h, sizeMat);
    memcpy(x1_d, x1_h, sizeVec);
    memcpy(x2_d, x2_h, sizeVec);
    memcpy(y1_d, y1_h, sizeVec);
    memcpy(y2_d, y2_h, sizeVec);
    ...
    args1[2] = (uint64_t)x1_d;
    args1[3] = (uint64_t)y1_d;
    ...
    args2[2] = (uint64_t)x2_d;
    args2[3] = (uint64_t)y2_d;
    args2[4] = (uint64_t)before2;
    args2[5] = (uint64_t)after2;
    ...
    hthread_group_exec(groupId, kernel1, 1, 5, args1);
    hthread_group_wait(groupId);
    ed = getCurrentTimeMicros(); tDsp1 = ed - st;

    st = getCurrentTimeMicros();
    hthread_group_exec(groupId, kernel2, 1, 5, args2);
    hthread_group_wait(groupId);
    ...
    /* -------------------- CPU 执行 -------------------- */
    st = getCurrentTimeMicros();
    mvt_cpu1(n, A_h, x1_h, y1_h);
    ed = getCurrentTimeMicros(); tCpu1 = ed - st;

    st = getCurrentTimeMicros();
    mvt_cpu2(n, A_h, x2_h, y2_h);
    ed = getCurrentTimeMicros(); tCpu2 = ed - st;

    /* -------------------- 校验结果 ------------------- */
    int err = check_result(n, x1_h, x1_d, x2_h, x2_d);
    if (err != 0) {
        fprintf(stderr, "MVT test FAILED!\n");
    } else {
        save_data("MVT", n, before1, after1, tDsp1, tCpu1,
                  clusterId, devProgram, nthreads, kernel1);
        save_data("MVT", n, before2, after2, tDsp2, tCpu2,
                  clusterId, devProgram, nthreads, kernel2);
        printf("WallTime MVT_kernel1 (DSP/CPU): %fs / %fs\n",
               tDsp1 / 1e6, tCpu1 / 1e6);
        printf("WallTime MVT_kernel2 (DSP/CPU): %fs / %fs\n",
               tDsp2 / 1e6, tCpu2 / 1e6);
    }
