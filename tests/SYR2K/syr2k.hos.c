 ...
 ************************************/
void syr2k_cpu(int ni, int nj, double alpha, double beta, double *A, double *B, double *C)
{
    for (int i = 0; i < ni; i++) {
        for (int j = 0; j < ni; j++) {
            C[i * ni + j] *= beta;
 ...
 ************************************/
void init_arrays(int ni, int nj, double *alpha, double *beta, double *A, double *B, double *C)
{
    *alpha = 32412;
    *beta = 2123;
...
{
    FILE *fp = fopen("tests/SYR2K/syr2k_events.txt", "a");
...
int main(int argc, char **argv)
{
    ...
    int    nj          = 1000; // NJ
    int    nthreads    = 1;
    char  *devProgram  = "operators/SYR2K/syr2k.dev.dat";
    char  *kernel      = "syr2k_kernel";
    ...
    size_t sizeMatAB = (size_t)ni * nj * sizeof(double);
    size_t sizeHot = 26 * sizeof(uint64_t);

    /* device */
    double  *A_d  = (double *)hthread_malloc(clusterId, sizeMatAB, HT_MEM_RO);
    double  *B_d  = (double *)hthread_malloc(clusterId, sizeMatAB, HT_MEM_RO);
    ...
    /* host */
    double *A_h = (double *)malloc(sizeMatAB);
    double *B_h = (double *)malloc(sizeMatAB);
    ...
    /* init data */
    double alpha, beta;
    init_arrays(ni, nj, &alpha, &beta, A_h, B_h, C_h);
    
    memcpy(A_d, A_h, sizeMatAB);
    memcpy(B_d, B_h, sizeMatAB);
    ...
    args[5] = (uint64_t)B_d;
    args[6] = (uint64_t)C_d;
    args[7] = (uint64_t)before;
    args[8] = (uint64_t)after;
    ...
    hthread_group_exec(groupId, kernel, 4, 5, args);
    ...
    /* -------------------- CPU 执行 -------------------- */
    st = getCurrentTimeMicros();
    syr2k_cpu(ni, nj, alpha, beta, A_h, B_h, C_h);
    ed = getCurrentTimeMicros(); tCpu = ed - st;

    /* -------------------- 校验结果 ------------------- */
    int err = check_result(ni, C_h, C_d);
    if (err != 0) {
        fprintf(stderr, "SYR2K test FAILED!\n");
    } else {
        save_data("SYR2K", ni, before, after, tDsp, tCpu,
                  clusterId, devProgram, nthreads, kernel);
        printf("WallTime SYR2K_kernel (DSP/CPU): %fs / %fs\n",
               tDsp / 1e6, tCpu / 1e6);
    }
